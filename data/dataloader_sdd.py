...
class SDDDataset(Dataset):
    def __init__(self, cfg, data_dir, 
        ...
        self.past_frames = cfg.past_frames # 8
        self.future_frames = cfg.future_frames # 12
        ...
        cfg.MODEL.CONTEXT_ENCODER.AGENTS = cfg.agents # 1
        
        ### compute past and future trajectories
        past_traj_abs = torch.from_numpy(np.stack([scene[0] for scene in all_data], axis=0)).unsqueeze(1) # 绝对位置   # [N, 1, T, 2]

        ### Compare with NSP model
        # past_traj_abs = torch.from_numpy(all_data[:,:self.past_frames])[...,:2].unsqueeze(1).float()    # [N, 1, T, 2]
        ### Compare with NSP model

        initial_pos = past_traj_abs[:, :, -1:, :]   # 观测最后一帧                    # [N, 1, 1, 2]
        self.initial_pos = initial_pos
        past_traj_rel = (past_traj_abs - initial_pos).contiguous()  # 相对位置        # [N, 1, T, 2]
        ...
        fut_traj_rel = (fut_traj_abs - initial_pos).contiguous()  # 相对位置                           # [N, 1, T, 2]
        ...
        self.fut_traj_vel = torch.cat((fut_traj_rel[:, :, 1:] - fut_traj_rel[:,:, :-1], torch.zeros_like(fut_traj_rel[:, :, -1:])), dim=2)  # 速度分量

        self.rotate_aug = cfg.rotate_aug and training # False  True
